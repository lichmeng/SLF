...
public class SearchResultListViewHolder extends BaseHolder<ProductListInfo> {
    ...
    public SearchResultListViewHolder(Context context) {
        super(context);
    }

    @Override
    protected View initView() {
        View view = View.inflate(mContext, R.layout.item_search_listview, null);
    ...
    public void bindData(ProductListInfo data) {
        mTvProductName.setText(data.getName() + "占位(积分卡类Smartisan似基地进发)字符 " + data.getName());
        mTvProductPrice.setText("¥"+ data.getPrice()+".00");
        mTvProductComment.setText("已有 "+new Random().nextInt(1000) + 500+" 人评价");
